  ...
  return (
    ...
    <article className="rounded-md overflow-hidden shadow-md hover:shadow-2xl">
      <Link href={`/articles/${id}`}>
        <Image src={`/images/thumbnails/${path}`} alt="dp" width={300} height={300} className="w-full max-h-48 object-fill"/>
        <div className="flex flex-col items-center p-4">
          ...
          <span className="text-sm rounded-lg bg-sky-300 px-2 my-2">{category.toUpperCase()}</span>
        </div>
      </Link>
    </article>
  );
